def climbingLeaderboard(scores, alice):
  pos = []
  scores = list(dict.fromkeys(scores))  #delete duplicates
  n = len(scores) #size of scores list 
  m = len(alice)  #size of alice's scores list

  counter = 0

  for i in range(n - 1, -1, -1):
    while alice[counter] <= scores[i]:
      if alice[counter] < scores[i]:
        pos.append(i + 2)
        counter += 1
      elif alice[counter] == scores[i]:
        pos.append(i + 1)
        counter += 1
      if counter >= m:
        return pos
...
alice = [int(i) for i in alice.split(' ')] 
